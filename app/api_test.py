...
class TestClass:
    ...
    def test_updating_user_data(self):
        data_put = requests.put("http://localhost:8000/api/v1/put", data=json.dumps(test_data))
        assert data_put.status_code==200
    def test_deleting_user_data(self):
        data_delete = requests.delete(f"http://localhost:8000/api/v1/delete/{user_id[0]}")
